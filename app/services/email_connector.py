...
class GmailOAuthConnector:
    ...
    def fetch_job_emails(self, days_back: int = 30, limit: int = 200) -> List[dict]:
        ...
        # Minimal search queries to prevent Render timeout (30s limit)
        # Only ~10 queries to ensure fast response
        search_queries = [
            # Most common: subject contains "application"
            f'after:{after_date} subject:application',
            # Interview-related
            f'after:{after_date} subject:interview',
            # Major job platforms (Indeed is most common)
